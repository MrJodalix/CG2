...
cbTimer(int lastCallTime)
{
  ...
  if ((*pause))
  {
    interval = (double)(thisCallTime/1000.0f) - pauseTime;
    calcLightPos(interval);
  }
  else
  {
    pauseTime = (double)(thisCallTime/1000.0f) - interval;
    calcLightPos(interval);
  }

  if ((*start))
  {
    calcEulerIntegration((thisCallTime - lastCallTime)/1000.0f);
  }
  else
  {
    calcEulerIntegration((0.0)/1000.0f);
  }
...
cbDisplay(void)
{
  ...
  GLdouble lookAt[9] = {sin(camera[0]) * cos(camera[1]) * camera[2], sin(camera[1]) * camera[2], cos(camera[0]) * cos(camera[1]) * camera[2], /* vorne */
                        0.0, 0.0, 0.0,  /* Zentrum */
