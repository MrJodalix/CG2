...
/**
 * Zeichnet die rollende Kugel.
 * @param ballPos die Position der Kugel
 */
void drawBallSphere(float *ballPos)
{
  /* Material */
  float matAmbient[] = {1.0, 1.0, 1.0, 1.0};
  float matDiffuse[] = {0.5, 0.5, 0.5, 1.0};
  ...
  glPushMatrix();
  {
    ...
    glTranslatef(ballPos[0] + ballPos[3] * RADIUS, ballPos[1] + ballPos[4] * RADIUS, ballPos[2] + ballPos[5] * RADIUS);
 ...
 * @param lightPos die Position der Lichtquelle
 */
...
/**
 * Zeichnet die Waende am Rand der Spline-Fläche.
 */
...
/**
 * zeichnet die kleinen Bloecke im Spielfeld
 */
void drawWalls(void)
{
  /* Material */
  ...
  glMaterialfv(GL_FRONT, GL_SPECULAR, matSpecular);
  glMaterialfv(GL_FRONT, GL_SHININESS, &matShininess);
  int *selectedWall = getSelectWall();
  for (int i = 0; i < 6; i++)
  {
    ...
    glPushMatrix();
    {
      ...
      glTranslatef((*wall)[0], (*wall)[1], (*wall)[2]);
      if (i == 2 || i == 5)
      {
        glScalef(WALL_SCALING, WALL_SCALING * 2.0, WALL_SCALING * 2.0);
      }
      else
      {
        glScalef(WALL_SCALING * 2.0, WALL_SCALING * 2.0, WALL_SCALING);
      }
      drawWall();
    }
    glPopMatrix();
...
void drawNormals()
{
  ...
  for (i = 0; i < (*numberOfInterPoints); i++)
  {
    glPushMatrix();
    {
      glColor3f(1.0f, 1.0f, 1.0f);
